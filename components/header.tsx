...
import { Menu, X, Facebook, Instagram, Linkedin, ShoppingBasket, Star, Home, Tag, MapPin, Phone } from "lucide-react"
          ...
          <nav className="hidden xl:flex items-center gap-6 absolute left-1/2 transform -translate-x-1/2">
            {navItems.map((item) => (
              ...
              >
                ...
                {hoveredTab === item.id && currentPage !== item.id && (
                  <motion.div
            ...
            >
              ...
              >
                {navItems.map((item) => (
                  <motion.button
                    ...
                    className={`w-full flex items-center gap-4 p-3.5 rounded-xl font-medium transition-all duration-200 border ${currentPage === item.id
                  ...
                  >
                    <div className={`p-2 rounded-lg ${currentPage === item.id ? "bg-primary/10 text-primary" : "bg-gray-100 text-gray-400"
                      }`}>
                      {/* Using generic icons mapped or defaults if refined mapping isn't available, visual consistency key */}
                      {item.id === 'home' && <Home className="w-5 h-5" />}
                      {item.id === 'encartes' && <Tag className="w-5 h-5" />}
                      {item.id === 'lojas' && <MapPin className="w-5 h-5" />}
                      {item.id === 'institucional' && <Star className="w-5 h-5" />}
                      {item.id === 'contato' && <Phone className="w-5 h-5" />}
                      {/* Fallback icon */}
                      {!['home', 'encartes', 'lojas', 'institucional', 'contato'].includes(item.id) && <div className="w-5 h-5 rounded-full border-2 border-current opacity-40" />}
                    </div>
                    <span className="text-base text-left flex-1">{item.label}</span>
                    {currentPage === item.id && <div className="w-1.5 h-1.5 rounded-full bg-secondary shadow-[0_0_8px_rgba(255,102,0,0.6)]" />}
                  </motion.button>
                ))}
              ...
              <div className="p-6 bg-white border-t border-gray-100">
                <div className="flex justify-center gap-5">
                  {/* Reusing the social links with smaller styling for footer */}
                  ...
                  <a href="#" className="text-gray-400 hover:text-blue-600 hover:scale-110 transition-all"><Linkedin className="w-6 h-6" /></a>
                </div>
